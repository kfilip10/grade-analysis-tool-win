  ...
  tabPanel(
    ...
    sidebarLayout(
      ...
      mainPanel(
        ...
        div(
          id = "course_checkbox_div", # style="display:none",
          dataTableOutput("courseCheckboxUITable")
        ),
        h3("2. Load and Select Assignments"),
        actionButton(
          inputId = "loadAssignmentsCanvas", label = "Load Assignments From Canvas",
          style = "color: black; background-color: #cfbb34"
        ),
        actionButton(
          inputId = "loadAssignmentsFromFile", label = "Load Assignments From Saved Data",
        ...
        disabled(actionButton(
          inputId = "load_canvas_data", label = "Load Grade and Roster Data from Canvas",
          style = "color: black; background-color: #cfbb34"
        )),
        tags$hr(),
...
canvasPrep_Handler <- function(input, output, session, canvas_api_token) {
  ...
  output$connectionStatus <- renderUI({
    ...
    if (!is.null(connection_status()) && connection_status() != "Success") {
      tagList(
        tags$p(
          style = "color: white;font-weight: bold;background-color: #781d1d;
    ...
    # If it does work then show the main UI for Canvas API access
      ...
      tagList(
        ...
        tags$p(
          style = "color: white;font-weight: bold;background-color: #6fbd7a;
                                text-align: center; border-radius: 10px; padding: 5px;",
  ...
  assign.group.comb <- reactive({
    req(course_list_df())
    if (!is.null(connection_status()) && connection_status() == "Success") {
      assign.group.comb <- course_list_df() %>%
        group_by(id) %>%
        group_split() %>%
        purrr::map_df(~ get_assignment_groups(.x, unique(.x$id)))
  ...
  observeEvent(input$load_canvas_data, {
    ...
    course.enrollment <- roster.course %>%
      group_by(course_id) %>%
      summarise(n = n())
      ...
      tryCatch(
        {
          ...
          gb.comb.parse_dt <- merge(gb.comb.parse_dt,
            assign.comb_dt,
            by = "assignment_group_id", all.x = TRUE
          )
  ...
  output$download_template_canvas <- downloadHandler(
    ...
    content = function(file) {
      ...
      list.template <- excel_template_from_canvas(
        loaded_course_df(),
        assignment_list_df(),
      ...
      tryCatch(
        {
          ...
          gb_sum <- summary_assign_groups_current %>%
            pivot_wider(
              names_from = assignment_group_name,
          ...
          gb_full <- roster.course %>%
            select(user_id, user.sortable_name, instructor, section_hour) %>%
          ...
          instructor_courses <- roster.course %>%
            group_by(course_id) %>%
  ...
  filteredPlotData <- reactive({
    ...
    course_gradebook() %>%
      group_by(section, assignment_group_name) %>%
  ...
  output$dataTableUpload <- renderDT(
    {
      ...
      if (!is.null(upload.list$dataframes)) {
        datatable(
          upload.list$dataframes[[1]] %>% select(
            ID, Section, Name, version,
