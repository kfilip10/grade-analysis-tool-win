  ...
  for (n in 1:numberVersions) {
    ...
    if (is.na(sheetN)) {
      ...
      if (is.na(sheetN)) {
        ...
        list.df[[n]] <- read_excel(file.excel, sheetN)
      }
    } else {
      list.df[[n]] <- read_excel(file.excel, sheetN)
    }
...
numquestions <- df.q %>%
  group_by(version) %>%
  summarise(questions = n_distinct(question))
...
apply(test, 1, function(x) {
  if (x[2] != x[3]) {
    ...
    str_c("Version ", x[1], "has a mismatched number of questions ")
  }
})
...
gb.csv <- c(
  "C:/Users/kev/OneDrive - West Point/6. R Projects/gb1.csv",
